Title: node-quickbooks

URL Source: https://www.npmjs.com/package/node-quickbooks

## Markdown Content:

image: https://static-production.npmjs.com/a12c728e90758ccd16976b394b964317.png
description: node.js client for Intuit&#x27;s IPP QuickBooks V3 API.. Latest version: 2.0.50, last published: 7 days ago. Start using node-quickbooks in your project by running `npm i node-quickbooks`. There are 44 other projects in the npm registry using node-quickbooks.
title: node-quickbooks
...
2.0.50 • Public • Published 7 days ago

- [ Readme](?activeTab=readme)
- [Code Beta](?activeTab=code)
- [10 Dependencies](?activeTab=dependencies)
- [44 Dependents](?activeTab=dependents)
...
The Intuit Developer Javascript code calls back into the node application, which needs to invoke the OAuth Request Token URL at <https://oauth.intuit.com/oauth/v1/get%5Frequest%5Ftoken> via a server-side http POST method. Note how the response from the http POST is parsed and the browser is redirected to the App Center URL at <https://appcenter.intuit.com/Connect/Begin?oauth%5Ftoken=> with the `oauth_token` passed as a URL parameter. Note also how the `oauth_token_secret` needs to somehow be maintained across http requests, as it needs to be passed in the second server-side http POST to the Access Token URL at <https://oauth.intuit.com/oauth/v1/get%5Faccess%5Ftoken>. This final step is invoked once the user has authenticated on Intuit's site and authorized the application, and then the user is redirected back to the node application at the callback URL specified as a parameter in the Request Token remote call, in the example app's case, <http://localhost:3000/callback>.
...
- [Intuit](/search?q=keywords:Intuit)
- [QuickBooks](/search?q=keywords:QuickBooks)
- [IPP](/search?q=keywords:IPP)
...
[Gitgithub.com/mcohen01/node-quickbooks](https://github.com/mcohen01/node-quickbooks)

### Homepage

[github.com/mcohen01/node-quickbooks](https://github.com/mcohen01/node-quickbooks)
...
[**Analyze security** with Socket](https://socket.dev/npm/package/node-quickbooks)[**Check bundle size**](https://bundlephobia.com/package/node-quickbooks)[**View package health**](https://snyk.io/advisor/npm-package/node-quickbooks)[**Explore dependencies**](https://npmgraph.js.org/?q=node-quickbooks)

[**Report** malware](/support?inquire=security&security-inquire=malware&package=node-quickbooks&version=2.0.50)
